...
-- FlyAndCo - Jeu de données TNR -> Nosy Be
-- Date: 2026-01-23
-- ============================================================
-- 
-- EXERCICE:
...
-- Tarif diffusion = 400 000 Ar / diffusion
-- AUCUN PAIEMENT DE DIFFUSION
-- ============================================================
...
DELETE FROM paiement WHERE id_facture IN (
    SELECT f.id_facture FROM facture f 
    WHERE f.numero_facture LIKE 'FAC-2026-%'
);

-- Supprimer les factures de test
DELETE FROM facture WHERE numero_facture LIKE 'FAC-2026-%';

-- Supprimer les réservations de test
DELETE FROM detail_reservation WHERE id_reservation IN (
...
DELETE FROM reservation WHERE id_vol_programme IN (
    SELECT vp.id_vol_programme FROM vol_programme vp
    JOIN avion a ON vp.id_avion = a.id_avion
    WHERE a.numero_immatriculation = 'TR-045'
);

-- Supprimer les vols programmés de test
DELETE FROM vol_programme WHERE id_avion = (
...
-- ============================================================
-- 3. CRÉATION DES SOCIÉTÉS PUBLICITAIRES (si pas existantes)
-- ============================================================
...
-- ============================================================
-- 5. CRÉATION DES VOLS PROGRAMMÉS ET DONNÉES
-- ============================================================
DO $$
DECLARE
    v_id_avion INT;
...
BEGIN
    ...
    -- ==========================================
    -- CRÉATION DES 3 VOLS PROGRAMMÉS
    -- ==========================================
    ...
    -- Vol 1 (20/01 10h): 40 billets adulte économique = 40 × 800 000 = 32 000 000 Ar
    ...
    END LOOP;
    RAISE NOTICE 'Vol 20/01 10h: 40 billets créés';
    
    -- Vol 2 (21/01 10h): 30 billets adulte économique = 30 × 800 000 = 24 000 000 Ar
    INSERT INTO reservation (id_client, id_vol_programme, nombre_places, statut)
    ...
    END LOOP;
    RAISE NOTICE 'Vol 21/01 10h: 30 billets créés';
    
    -- Vol 3 (21/01 15h): 50 billets adulte économique = 50 × 800 000 = 40 000 000 Ar
    INSERT INTO reservation (id_client, id_vol_programme, nombre_places, statut)
    ...
    FOR i IN 1..50 LOOP
        ...
        VALUES (v_id_reservation, 2, 1, 800000);
    END LOOP;
    RAISE NOTICE 'Vol 21/01 15h: 50 billets créés';

    -- ==========================================
    -- FACTURES DE DIFFUSION (SANS PAIEMENT)
    -- ==========================================
    ...
    RAISE NOTICE 'Facture Vaniala créée (vol 20/01): 1 diffusion, 400 000 Ar';

    -- Vol 20/01 10h: Pub Lewis 1 (400 000 Ar) - NON PAYÉE
    INSERT INTO facture (numero_facture, id_societe_diffuseur, date_facture, date_debut_periode, date_fin_periode, montant, statut)
    VALUES ('FAC-2026-L001', v_id_societe_lewis, '2026-01-20', '2026-01-20', '2026-01-20', 400000, 'émise')
    RETURNING id_facture INTO v_id_facture;
    
    INSERT INTO detail_facture (id_facture, id_vol_programme, description, nombre_diffusions, prix_unitaire, montant_ligne, montant_paye)
    VALUES (v_id_facture, v_id_vp_1, 'Diffusion pub vol TNR-NOS 20/01 10h', 1, 400000, 400000, 0);
    RAISE NOTICE 'Facture Lewis créée (vol 20/01): 1 diffusion, 400 000 Ar';

    -- Vol 21/01 10h: Pub Socobis 2 (800 000 Ar) - NON PAYÉE
    INSERT INTO facture (numero_facture, id_societe_diffuseur, date_facture, date_debut_periode, date_fin_periode, montant, statut)
    VALUES ('FAC-2026-S001', v_id_societe_socobis, '2026-01-21', '2026-01-21', '2026-01-21', 800000, 'émise')
    RETURNING id_facture INTO v_id_facture;
    
    INSERT INTO detail_facture (id_facture, id_vol_programme, description, nombre_diffusions, prix_unitaire, montant_ligne, montant_paye)
    VALUES (v_id_facture, v_id_vp_2, 'Diffusion pub vol TNR-NOS 21/01 10h', 2, 400000, 800000, 0);
    RAISE NOTICE 'Facture Socobis créée (vol 21/01): 2 diffusions, 800 000 Ar';

    -- Vol 21/01 10h: Pub Jejoo 1 (400 000 Ar) - NON PAYÉE
    INSERT INTO facture (numero_facture, id_societe_diffuseur, date_facture, date_debut_periode, date_fin_periode, montant, statut)
    ...
    RAISE NOTICE 'Facture Jejoo créée (vol 21/01): 1 diffusion, 400 000 Ar';

    -- Vol 21/01 15h: 0 diffusion (pas de facture)
    RAISE NOTICE 'Vol 21/01 15h: 0 diffusion';
...
SELECT '=== RÉSUMÉ ATTENDU ===' AS info;
SELECT 
    'Vol' AS type,
    TO_CHAR(vp.date_heure_depart, 'DD/MM/YYYY') AS date,
    TO_CHAR(vp.date_heure_depart, 'HH24"h"MI') AS heure,
    COUNT(DISTINCT dr.id_detail_reservation) AS nb_billets,
    TO_CHAR(COALESCE(SUM(dr.prix_paye), 0), 'FM999 999 999') || ' Ar' AS ca_billets,
    COALESCE((SELECT SUM(df2.nombre_diffusions) FROM detail_facture df2 
              JOIN facture f2 ON df2.id_facture = f2.id_facture 
              WHERE df2.id_vol_programme = vp.id_vol_programme AND f2.statut != 'annulée'), 0) AS nb_diffusions,
    TO_CHAR(COALESCE((SELECT SUM(df2.montant_ligne) FROM detail_facture df2 
              JOIN facture f2 ON df2.id_facture = f2.id_facture 
              WHERE df2.id_vol_programme = vp.id_vol_programme AND f2.statut != 'annulée'), 0), 'FM999 999 999') || ' Ar' AS total_diffusions,
    TO_CHAR(COALESCE((SELECT SUM(df2.montant_ligne - df2.montant_paye) FROM detail_facture df2 
              JOIN facture f2 ON df2.id_facture = f2.id_facture 
              WHERE df2.id_vol_programme = vp.id_vol_programme AND f2.statut != 'annulée'), 0), 'FM999 999 999') || ' Ar' AS reste_diffusions
...
-- RÉSULTAT ATTENDU:
-- ============================================================
-- | Date       | Heure | Billets | CA Billets    | Diff | Total Diff  | Reste Diff  |
-- |------------|-------|---------|---------------|------|-------------|-------------|
-- | 20/01/2026 | 10h00 | 40      | 32 000 000 Ar | 2    | 800 000 Ar  | 800 000 Ar  |
-- | 21/01/2026 | 10h00 | 30      | 24 000 000 Ar | 3    | 1 200 000 Ar| 1 200 000 Ar|
-- | 21/01/2026 | 15h00 | 50      | 40 000 000 Ar | 0    | 0 Ar        | 0 Ar        |
-- |------------|-------|---------|---------------|------|-------------|-------------|
-- | TOTAL      |       | 120     | 96 000 000 Ar | 5    | 2 000 000 Ar| 2 000 000 Ar|
-- ============================================================
